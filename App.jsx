...
import {
  ChevronRight, ChevronLeft, Moon, Sun, RotateCcw, Info,
  CheckCircle, Zap, Target, Users, TrendingUp, BarChart2,
  ArrowRight, AlertTriangle, BookOpen, GitBranch, Lightbulb,
  Copy, ChevronDown,
} from "lucide-react";
...
const TIPS = {
  ...
  normality:
    "For t-tests and ANOVA, normality refers to the residuals — not the raw variable. With n>30, the Central Limit Theorem makes parametric tests robust.",
  mediation: "A mediator (M) explains the mechanism: X→M→Y.",
  "composite scale":
    "Built from multiple items (e.g. PHQ-9, GAD-7). When items are averaged with n>30, the result is treated as continuous — parametric tests are widely accepted (Norman, 2010).",
};

const BC = {
  Parametric: ["#ddd6fe", "#3730a3"],
  "Non-Parametric": ["#dcfce7", "#166534"],
  Advanced: ["#ede9fe", "#581c87"],
};

const T = {
  independent_ttest: {
    n: "Independent Samples t-Test", e: "👥", b: "Parametric", c: 95,
    tl: "Compare two separate groups on a continuous outcome",
    why: "Compares means between two independent, normally distributed groups — the gold standard for this design.",
    ass: ["Continuous DV", "Normal distribution per group (or n>30 by CLT)", "Homogeneity of variance — Levene's test", "Independent observations"],
    ...
    jmv: "T-Tests → Independent Samples T-Test",
  },
  paired_ttest: {
    n: "Paired Samples t-Test", e: "🔄", b: "Parametric", c: 95,
    tl: "Track changes in the same participants over time",
    why: "Analyses difference scores within the same participants — more powerful than an independent t-test.",
  ...
  oneway_anova: {
    n: "One-Way ANOVA", e: "📊", b: "Parametric", c: 93,
    ...
    ph: "Tukey HSD or Bonferroni; Games-Howell if variances differ",
    spss: "Analyze → Compare Means → One-Way ANOVA → Post Hoc",
    jmv: "ANOVA → One-Way ANOVA",
  },
  rm_anova: {
    n: "Repeated Measures ANOVA", e: "🔁", b: "Parametric", c: 93,
    ...
    ass: ["Continuous DV", "Sphericity — Mauchly's test (Greenhouse-Geisser if violated)", "Approximately normal residuals"],
    alt: { n: "Friedman Test", w: "when sphericity or normality is violated" },
    eff: "Partial η²p or generalised η²G",
  ...
  mann_whitney: {
    n: "Mann-Whitney U Test", e: "🏅", b: "Non-Parametric", c: 91,
    tl: "Compare two groups without assuming normality",
    why: "Your data doesn't quite fit the standard mould — Mann-Whitney compares rank distributions and is fully distribution-free.",
    ass: ["Ordinal or continuous DV", "Independent observations", "Similar distribution shapes (for median comparison)"],
    alt: { n: "Independent Samples t-Test", w: "when normality and equal variances are met" },
  ...
  wilcoxon: {
    n: "Wilcoxon Signed-Rank Test", e: "✍️", b: "Non-Parametric", c: 90,
  ...
  kruskal_wallis: {
    n: "Kruskal-Wallis Test", e: "📈", b: "Non-Parametric", c: 90,
  ...
  friedman: {
    n: "Friedman Test", e: "🌀", b: "Non-Parametric", c: 89,
  ...
  pearson: {
    n: "Pearson Correlation (r)", e: "📉", b: "Parametric", c: 96,
    tl: "Measure linear relationship between two continuous variables",
    why: "Both variables are continuous and approximately normal — Pearson r quantifies the strength and direction (−1 to +1) of the linear relationship.",
    ass: ["Both variables continuous", "Approximately bivariate normal", "Linear relationship — check scatter plot", "No significant outliers"],
    alt: { n: "Spearman's rho (ρ)", w: "when normality is violated or variables are ordinal" },
  ...
  spearman: {
    n: "Spearman's Rank Correlation (ρ)", e: "🔗", b: "Non-Parametric", c: 93,
    ...
    alt: { n: "Pearson r", w: "when both variables are normally distributed and linearly related" },
  ...
  simple_regression: {
    n: "Simple Linear Regression", e: "📐", b: "Parametric", c: 94,
    tl: "Predict a continuous outcome from one predictor",
    why: "Estimates the linear relationship between one predictor and a continuous outcome, quantifying change per unit of the predictor.",
    ass: ["Linear relationship", "Normal residuals (not the raw data)", "Homoscedasticity", "Independent observations"],
  ...
  multiple_regression: {
    n: "Multiple Linear Regression", e: "🔮", b: "Parametric", c: 93,
    tl: "Predict a continuous outcome from multiple predictors",
    why: "Partials out each predictor's unique contribution while controlling for all others — essential when predictors correlate.",
  ...
  logistic_regression: {
    n: "Binary Logistic Regression", e: "⚖️", b: "Parametric", c: 93,
    ...
    ass: ["Binary DV", "Log-odds linearity with continuous predictors", "No multicollinearity", "≥10 events per predictor", "Independent observations"],
  ...
  multinomial_logistic: {
    n: "Multinomial Logistic Regression", e: "🗂️", b: "Parametric", c: 90,
    ...
    ass: ["Nominal outcome with 3+ categories", "No multicollinearity", "Adequate sample per category"],
  ...
  chi_square: {
    n: "Chi-Square Test (χ²)", e: "🔲", b: "Non-Parametric", c: 92,
    ...
    ass: ["Both variables categorical", "Independent observations", "Expected cell frequencies ≥5", "Random sampling"],
  ...
  mediation_moderation: {
    n: "Mediation / Moderation Analysis", e: "🔀", b: "Advanced", c: 88,
    tl: "Unpack how and when relationships occur",
    why: "Reveals mechanisms (mediation: X→M→Y) or boundary conditions (moderation: when does X→Y change?) behind relationships.",
    ...
    eff: "Mediation: indirect effect (a×b); Moderation: interaction coefficient β₃",
    ex: "Self-efficacy mediating social support → academic performance (Hayes, 2022).",
    viz: "Path diagram; interaction plot (Y vs X at levels of W)",
    sw: "PROCESS Macro v4 (Hayes, 2022) — most widely used in psychology.",
...
function recommend(a) {
  ...
  if (a.objective === "compare") {
    ...
    if (a.groups === "2") {
      ...
      return ok ? "independent_ttest" : "mann_whitney";
    }
...
function nextQ(q, a) {
  ...
  if (q === "dv_type") {
    ...
    if (a.dvType === "continuous" || a.dvType === "ordinal") return "norm_n";
    return "result";
  }
...
function Tip({ term, dark }) {
  ...
  return (
    <span style={{ position: "relative", display: "inline-flex" }}>
      <button onMouseEnter={() => setS(true)} onMouseLeave={() => setS(false)} onClick={e => { e.stopPropagation(); setS(v => !v); }} style={{ background: "none", border: "none", cursor: "pointer", padding: "0 2px", color: "#818cf8", lineHeight: 1 }}>
        <Info size={11} />
      </button>
      {s && <span style={{ position: "absolute", bottom: "calc(100% + 4px)", left: 0, zIndex: 99, width: 185, fontSize: 11, lineHeight: 1.6, borderRadius: 10, padding: "8px 10px", background: "#1e293b", color: "#e2e8f0", boxShadow: "0 8px 24px rgba(0,0,0,.25)", pointerEvents: "none" }}>{TIPS[term]}</span>}
    </span>
  );
}
...
function Opt({ o, sel, onSel, dark }) {
  return (
    <button onClick={() => onSel(o.id)} style={{ width: "100%", textAlign: "left", padding: "12px 14px", borderRadius: 14, border: sel ? "2px solid #6366f1" : `1.5px solid ${dark ? "#334155" : "#e2e8f0"}`, background: sel ? (dark ? "rgba(99,102,241,.15)" : "rgba(99,102,241,.06)") : (dark ? "#1e293b" : "#fff"), cursor: "pointer", transition: "all .15s", boxShadow: sel ? "0 0 0 3px rgba(99,102,241,.15)" : "none" }}>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", flexShrink: 0 }}>
      <svg width="66" height="66" viewBox="0 0 66 66">
        <defs><linearGradient id="rg"><stop offset="0%" stopColor="#6366f1" /><stop offset="100%" stopColor="#3b82f6" /></linearGradient></defs>
        <circle cx="33" cy="33" r={r} fill="none" stroke={dark ? "#334155" : "#e2e8f0"} strokeWidth="6" />
        <circle cx="33" cy="33" r={r} fill="none" stroke="url(#rg)" strokeWidth="6" strokeDasharray={`${fill} ${ci}`} strokeDashoffset={ci / 4} strokeLinecap="round" />
        <text x="33" y="38" textAnchor="middle" fontSize="13" fontWeight="700" fill={dark ? "#c7d2fe" : "#4338ca"}>{pct}%</text>
      </svg>
      <span style={{ fontSize: 10, fontWeight: 500, color: dark ? "#64748b" : "#94a3b8", marginTop: -4 }}>confidence</span>
    </div>
  );
}
...
function Crumb({ hist, ans, dark }) {
  ...
  return (
    <div style={{ marginBottom: 14, padding: "8px 12px", borderRadius: 12, background: dark ? "rgba(99,102,241,.06)" : "rgba(99,102,241,.04)", border: `1px solid ${dark ? "rgba(99,102,241,.2)" : "rgba(99,102,241,.12)"}` }}>
      <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: "3px 5px" }}>
        {chips.map((c, i) => (<span key={i} style={{ display: "flex", alignItems: "center", gap: 4 }}><span style={{ fontSize: 11, fontWeight: 500, color: dark ? "#94a3b8" : "#64748b", padding: "2px 7px", borderRadius: 99, background: dark ? "#334155" : "#f1f5f9" }}>{c}</span>{i < chips.length - 1 && <span style={{ fontSize: 10, color: tx3 }}>→</span>}</span>))}
      </div>
    </div>
  );
}
